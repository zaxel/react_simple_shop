...
const CartProducts = ({ items, setItems }) => {
    return (
        <div className='w-full lg:w-7/12 shadow-lg border-1 border-gray-100 p-8 rounded-lg flex flex-col gap-8'>
            {items.length 
                ? items.map(({ id, name, img, price, rate, type, brand, quantity, createdAt }) => {
                return <div className='flex justify-between items-center' key={id}>
                    <div className='flex gap-8'>
                        <div className=''>
                            <div className='relative w-32 h-32 bg-gray-50 rounded-lg overflow-hidden'>
                                <img className='absolute w-full h-full object-cover' src={img} alt={name} />
                            </div>
                        </div>
                    ...
                    <button onClick={()=>setItems(prev => [...prev].filter(item=>item.id!==id))} className='w-8 h-8 shrink-0 flex justify-center items-center rounded-full bg-red-100 text-red-400 cursor-pointer'><Trash2 className='w-3 h-3' /></button>
                </div>
            })
