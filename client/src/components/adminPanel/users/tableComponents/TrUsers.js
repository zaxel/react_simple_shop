...
const TrUsers = ({ data }) => {
    ...
    return (
        <tr >
            <td>{data.id}</td>
            <TdInputText toolTipInfo={toolTipInfo0} innerRef={ref0} data={{inputData: data.email, id: data.id, dbFieldName: 'email'}}/>
            <TdUserRoleSelect toolTipInfo={toolTipInfo1} innerRef={ref1} data={{inputData: data.role, id: data.id, dbFieldName: 'role'}}/>
            <TdIsActivatedSelect toolTipInfo={toolTipInfo2} innerRef={ref2} data={{inputData: data.is_activated.toString(), id: data.id, dbFieldName: 'is_activated'}}/>
            <td>{adminPageFormatDate(data.createdAt)}</td>
            <TdModalLink toolTipInfo={toolTipInfo3} innerRef={ref3} data={data.onOrderClickHandler.bind(this, data.id)}/>
            <TdDelete toolTipInfo={toolTipInfo4} innerRef={ref4} data={{id: data.id}}/>
        </tr>
    );
};
